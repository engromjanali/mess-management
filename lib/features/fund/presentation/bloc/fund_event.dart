...
class FundEvent with _$FundEvent {
  ...
  const factory FundEvent.changeDateFilter(FundDateFilter filter) = FundChangeDateFilter;
  ...
  const factory FundEvent.selectRange({required DateTime start, required DateTime end}) = FundSelectRange;

  /// Record a fund entry. [amount] is signed
  /// (positive → credit, negative → debit).
  const factory FundEvent.add({required double amount, required DateTime date, String? note}) = FundAdd;

  /// Edit an existing fund entry.
  const factory FundEvent.update({required String id, required double amount, required DateTime date, String? note}) = FundUpdate;
