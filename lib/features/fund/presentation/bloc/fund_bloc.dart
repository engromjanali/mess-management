...
class FundBloc extends Bloc<FundEvent, FundState> {
  ...
  FundBloc(this._getAllFunds, this._getFundsByDate, this._getFundsInRange, this._addFund, this._updateFund, this._deleteFund) : super(const FundState.initial()) {
  ...
  Future<void> _onStarted(FundStarted event, Emitter<FundState> emit) async {
    _isAdmin = event.isAdmin;
    emit(const FundState.loading());
    await _reload(emit);
  }

  Future<void> _onChangeDateFilter(FundChangeDateFilter event, Emitter<FundState> emit) async {
    _dateFilter = event.filter;
    emit(const FundState.loading());
    await _reload(emit);
  }

  Future<void> _onSelectDate(FundSelectDate event, Emitter<FundState> emit) async {
  ...
  Future<void> _onSelectRange(FundSelectRange event, Emitter<FundState> emit) async {
  ...
  Future<void> _onAdd(FundAdd event, Emitter<FundState> emit) async {
    _emitSaving(emit);
    final result = await _addFund(AddFundParams(amount: event.amount, date: event.date, note: event.note));
    await _afterMutation(emit, result);
  }

  Future<void> _onUpdate(FundUpdate event, Emitter<FundState> emit) async {
    _emitSaving(emit);
    final result = await _updateFund(UpdateFundParams(id: event.id, amount: event.amount, date: event.date, note: event.note));
    await _afterMutation(emit, result);
  }
  ...
  Future<void> _reload(Emitter<FundState> emit) async {
    ...
    switch (_dateFilter) {
      ...
      case FundDateFilter.range:
        ...
        result = range == null ? await _getAllFunds(const NoParams()) : await _getFundsInRange(FundRangeParams(start: range.start, end: range.end));
    }

    result.when(success: (s) => emit(_loaded(s.data)), failure: (f) => emit(FundState.error(f.error.toString())));
  }
  ...
  FundState _loaded(List<FundEntity> funds) => FundState.loaded(funds: funds, dateFilter: _dateFilter, selectedDate: _selectedDate, isAdmin: _isAdmin, selectedRange: _selectedRange);
}
