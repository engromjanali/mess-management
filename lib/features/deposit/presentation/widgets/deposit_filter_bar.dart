...
class DepositFilterBar extends StatelessWidget {
  const DepositFilterBar({required this.mode, required this.members, required this.selectedMember, required this.dateFilter, required this.selectedDate, required this.selectedRange, super.key});
  ...
  Future<void> _pickDay(BuildContext context) async {
    final picked = await showDatePicker(context: context, initialDate: selectedDate, firstDate: DateTime(selectedDate.year - 2), lastDate: DateTime(selectedDate.year + 2));
  ...
  Future<void> _pickRange(BuildContext context) async {
    ...
    final picked = await showDateRangePicker(context: context, initialDateRange: selectedRange, firstDate: DateTime(now.year - 2), lastDate: DateTime(now.year + 2));
    if (picked != null && context.mounted) {
      _bloc(context).add(DepositEvent.selectRange(start: picked.start, end: picked.end));
    }
  }
    ...
    return Wrap(
      ...
      children: [
        ...
        if (byMember)
          ...
          _SelectChip(
            label: dateFilter == DepositDateFilter.range && selectedRange != null ? DepositFormatters.rangeLabel(selectedRange!.start, selectedRange!.end) : 'Range',
...
class _SelectChip extends StatelessWidget {
  const _SelectChip({required this.label, required this.selected, required this.onTap, this.icon});
  ...
  Widget build(BuildContext context) {
    ...
    return Material(
      color: selected ? colors.primaryColor.withValues(alpha: 0.12) : colors.cardBackgroundColor,
      ...
      child: InkWell(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            borderRadius: BorderRadius.circular(Dimensions.radiusExtraLarge),
            border: Border.all(color: selected ? colors.primaryColor : colors.borderColor, width: selected ? 1.4 : 1),
          ),
          child: Row(
            mainAxisSize: MainAxisSize.min,
            children: [
              if (icon != null) ...[Icon(icon, size: Dimensions.iconSizeSmall, color: fg), const SizedBox(width: Dimensions.paddingSizeExtraSmall)],
              Text(
                label,
                style: AppTextStyles.sfProRoundedSemiBold.copyWith(fontSize: Dimensions.fontSizeSmall, color: fg),
              ),
            ],
          ),
        ),
...
class _MemberMenuChip extends StatelessWidget {
  const _MemberMenuChip({required this.members, required this.selectedMember, required this.onSelected});
    ...
    return PopupMenuButton<String?>(
      ...
      child: Container(
        ...
        child: Row(
          mainAxisSize: MainAxisSize.min,
          children: [
            Icon(Icons.groups_rounded, size: Dimensions.iconSizeSmall, color: colors.primaryColor),
            ...
            Icon(Icons.arrow_drop_down_rounded, size: Dimensions.iconSizeDefault, color: colors.primaryColor),
          ],
        ),
      ),
    );
