...
class DepositEvent with _$DepositEvent {
  ...
  const factory DepositEvent.changeMode(DepositViewMode mode) = DepositChangeMode;

  /// Show deposits for [memberId] (switches to by-member mode). A null
  /// [memberId] means "all members".
  const factory DepositEvent.selectMember(String? memberId) = DepositSelectMember;

  /// Switch the by-date scope (e.g. to "all time").
  const factory DepositEvent.changeDateFilter(DepositDateFilter filter) = DepositChangeDateFilter;
  ...
  const factory DepositEvent.selectRange({required DateTime start, required DateTime end}) = DepositSelectRange;

  /// Record a deposit for one member. [amount] is signed
  /// (positive → credit, negative → debit).
  const factory DepositEvent.add({required String memberId, required double amount, required DateTime date, String? note}) = DepositAdd;

  /// Edit an existing deposit.
  const factory DepositEvent.update({required String id, required double amount, required DateTime date, String? note}) = DepositUpdate;
