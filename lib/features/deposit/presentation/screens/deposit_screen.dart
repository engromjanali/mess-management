...
class DepositScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return BlocProvider<DepositBloc>(
      create: (_) =>
          getIt<DepositBloc>()..add(DepositEvent.started(isAdmin: isAdmin)),
...
class _DepositView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return BlocListener<RoleCubit, UserRole>(
      ...
      listener: (context, role) => context
          .read<DepositBloc>()
          .add(DepositEvent.started(isAdmin: role.isAdmin)),
      child: Scaffold(
        ...
        floatingActionButton: BlocBuilder<DepositBloc, DepositState>(
          builder: (context, state) {
            final canAdd = state.maybeWhen(
              loaded: (_, _, _, _, _, isAdmin, _, _, _) => isAdmin,
              orElse: () => false,
            );
            if (!canAdd) return const SizedBox.shrink();
            return FloatingActionButton.extended(
              onPressed: () => _openAdd(context, state),
        ...
        body: BlocConsumer<DepositBloc, DepositState>(
          ...
          builder: (context, state) {
            return state.maybeWhen(
              ...
              loaded: (mode, deposits, members, dateFilter, selectedDate,
                      isAdmin, selectedMember, selectedRange, saving) =>
                  _DepositBody(
                mode: mode,
                deposits: deposits,
                members: members,
  ...
  void _openAdd(BuildContext context, DepositState state) {
    final members = state.maybeWhen(
      loaded: (_, _, members, _, _, _, _, _, _) => members,
      orElse: () => const <DepositMemberEntity>[],
    );
    final bloc = context.read<DepositBloc>();
    showDepositFormSheet(
      context: context,
      members: members,
      onSave: ({required memberId, required amount, required date, note}) =>
          bloc.add(DepositEvent.add(
        memberId: memberId,
        amount: amount,
        date: date,
        note: note,
        ...
        child: Column(
          ...
          children: [
            ...
            ElevatedButton.icon(
              onPressed: () => context
                  .read<DepositBloc>()
                  .add(DepositEvent.started(isAdmin: isAdmin)),
...
class _DepositBody extends StatelessWidget {
  ...
  List<_Stat> _summary(BuildContext context) {
    ...
    return [
      _Stat('Total Credit', DepositFormatters.taka(deposits.totalCredit),
          Icons.south_west_rounded, c.successColor),
      _Stat('Total Debit', DepositFormatters.taka(deposits.totalDebit),
          Icons.north_east_rounded, c.errorColor),
      _Stat('Net Balance', DepositFormatters.signedTaka(net),
          Icons.account_balance_wallet_rounded,
          net < 0 ? c.errorColor : c.primaryColor),
      _Stat('Entries', '${deposits.length}', Icons.receipt_long_rounded,
          c.infoColor),
    ];
  }
  ...
  Widget build(BuildContext context) {
    ...
    final showMember = mode == DepositViewMode.byDate ||
        (mode == DepositViewMode.byMember && selectedMember == null);
                  ...
                  children: [
                    ...
                    if (deposits.isEmpty)
                      ...
                      _DepositList(
                        ...
                        isAdmin: isAdmin,
                        onEdit: (d) => _onEdit(context, d),
                        onDelete: (d) => _onDelete(context, d),
                      ),
    ...
    switch (mode) {
      case DepositViewMode.byMember:
        return selectedMember == null
            ? 'All members'
            : '${selectedMember!.name}\'s deposits';
      case DepositViewMode.byDate:
        switch (dateFilter) {
          ...
          case DepositDateFilter.range:
            ...
            return r == null
                ? 'All deposits'
                : 'Deposits · ${DepositFormatters.rangeLabel(r.start, r.end)}';
        }
          ...
          for (var i = 0; i < deposits.length; i++)
            AnimatedEntrance(
              ...
              child: DepositTile(
                ...
                showMember: showMember,
                showActions: isAdmin,
                onEdit: () => onEdit(deposits[i]),
                onDelete: () => onDelete(deposits[i]),
              ),
            ),
        ];
