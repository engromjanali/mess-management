...
class DepositRepositoryImpl implements DepositRepository {
  ...
  @override
  ResultFuture<List<DepositEntity>> getMemberDeposits(String memberId) => _guard(() async {
    final models = await _dataSource.getMemberDeposits(memberId);
    return models.map((m) => m.toEntity()).toList();
  });

  @override
  ResultFuture<List<DepositEntity>> getDepositsByDate(DateTime date) => _guard(() async {
    final models = await _dataSource.getDepositsByDate(date);
    return models.map((m) => m.toEntity()).toList();
  });

  @override
  ResultFuture<List<DepositEntity>> getDepositsInRange(DateTime start, DateTime end) => _guard(() async {
  ...
  @override
  ResultFuture<DepositEntity> addDeposit({required String memberId, required double amount, required DateTime date, String? note}) => _guard(() async {
    final model = await _dataSource.addDeposit(memberId: memberId, amount: amount, date: date, note: note);
    return model.toEntity();
  });

  @override
  ResultFuture<DepositEntity> updateDeposit({required String id, required double amount, required DateTime date, String? note}) => _guard(() async {
    final model = await _dataSource.updateDeposit(id: id, amount: amount, date: date, note: note);
    return model.toEntity();
  });

  @override
  ResultVoid deleteDeposit(String id) => _guard(() => _dataSource.deleteDeposit(id));
