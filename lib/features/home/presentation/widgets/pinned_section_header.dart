...
class PinnedSectionHeader extends SliverPersistentHeaderDelegate {
  PinnedSectionHeader({required this.title, this.icon, this.trailing, this.horizontalPadding = Dimensions.paddingSizeLarge});
  ...
  Widget build(BuildContext context, double shrinkOffset, bool overlapsContent) {
    ...
    return AnimatedContainer(
      ...
      decoration: BoxDecoration(
        color: pinned ? context.theme.scaffoldBackgroundColor : context.theme.scaffoldBackgroundColor.withValues(alpha: 0),
        boxShadow: pinned ? [BoxShadow(color: colors.shadowColor, blurRadius: 8, offset: const Offset(0, 4))] : null,
      ),
  ...
  bool shouldRebuild(covariant PinnedSectionHeader oldDelegate) {
    return oldDelegate.title != title || oldDelegate.icon != icon || oldDelegate.trailing != trailing || oldDelegate.horizontalPadding != horizontalPadding;
  }
}
