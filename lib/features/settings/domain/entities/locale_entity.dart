...
class AppLocale {
  ...
  static List<Locale> get supportedLocales {
    return AppConstants.languages.map((language) => Locale(language.code)).toList();
  }

  /// Get locale from language code
  static Locale fromCode(String code) {
    // Check if the code exists in our languages list
    final languageExists = AppConstants.languages.any((language) => language.code == code);
  ...
  static String getLanguageName(Locale locale) {
    try {
      final language = AppConstants.languages.firstWhere((lang) => lang.code == locale.languageCode);
